...
class Component extends DCLogic {
  ...
  _applySeeds(){
    const V = 'contacts-csv-2026-06-27';
  ...
  renderVals(){
    ...
    const addContact = () => {
      ...
      this.setState(s=>({ contactsData:[{brand:company, person:(person||'—'), role:(this.state.ncoRole||'').trim(), tag:type, email:(this.state.ncoEmail||'').trim(), phone:(this.state.ncoPhone||'').trim(), last:'jamais', deals:'—', tone:_typeTones[type]||'indigo'}].concat(s.contactsData||this.contactRaw), showContactForm:false, ncoFirst:'', ncoLast:'', ncoCompany:'', ncoRole:'', ncoEmail:'', ncoPhone:'', ncoType:'Marque' }));
