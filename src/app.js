...
class Component extends DCLogic {
  ...
  _cloudKeys(){ const local={deletedRoster:1,rosterEdited:1,prospectData:1,contactsData:1,invoiceData:1,photos:1,authed:1,authRole:1,space:1,creatorId:1,portalTab:1}; return this._persistKeys().filter(k=> !local[k]); }
  _persist(){
  ...
  renderVals(){
    ...
    return {
      ...
      addBrief:()=>{ const br=(this.state.nbBrand||'').trim(); if(!br)return; const ci=this.state.nbCreator||0; const nm=this.rosterRaw[ci].name; const tone=this.rosterRaw[ci].tone; const consignes=(this.state.nbConsignes||'').trim(); const budget=(this.state.nbBudget||'').trim()||'\u2014'; const objectif=(this.state.nbObjectif||'').trim()||'\u2014'; const deliverables=this.state.nbDeliv||'\u2014'; const due=this.state.nbDue||'\u2014'; const editKey=this.state.editBrief;
        if(editKey){ this.setState(s=>({ briefItems:(s.briefItems||this.briefRaw).map(x=> x.brand===editKey ? Object.assign({},x,{brand:br, creator:nm, who:nm, tone, deliverables, due, consignes, budget, objectif}) : x), showBriefForm:false, editBrief:null, briefOpen:br, nbBrand:'', nbDeliv:'', nbDue:'', nbConsignes:'', nbBudget:'', nbObjectif:'' })); toast('Brief modifi\u00e9 \u2713'); return; }
        const cur=(this.state.briefItems||this.briefRaw).slice(); cur.unshift({brand:br, creator:nm, deliverables, due, status:'attente', tone:tone, who:nm, consignes, budget, objectif}); this.setState({briefItems:cur, showBriefForm:false, nbBrand:'', nbDeliv:'', nbDue:'', nbConsignes:'', nbBudget:'', nbObjectif:''}); },
