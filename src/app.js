...
class Component extends DCLogic {
  ...
  renderVals(){
    ...
    const myAgenda = myEvents.map(e => { const d=eventDeco(e); return { day:e.day, time:e.time, title:e.title, dotStyle:d.dotStyle, dayBoxStyle:'width:46px;flex-shrink:0;text-align:center;background:var(--rowhover);border-radius:10px;padding:6px 0;color:var(--text);', edit:(ev)=>{ if(ev&&ev.stopPropagation)ev.stopPropagation(); const _n=new Date(); const date=_n.getFullYear()+'-'+String(_n.getMonth()+1).padStart(2,'0')+'-'+String(e.day).padStart(2,'0'); this.setState({ showEventForm:true, editingEvent:e, ne:{ day:e.day, date:date, time:(e.time&&e.time!=='—'?e.time:''), title:e.title, type:e.type||'call' } }); }, del:(ev)=>{ if(ev&&ev.stopPropagation)ev.stopPropagation(); if(!window.confirm('Supprimer cet événement ?'))return; if(this._eventsTable && e.id) this._dbDelete('events', e.id); this.setState(s=>({ events:(s.events||this.eventsRaw).filter(x=>x!==e) })); } }; });
