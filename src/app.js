...
class Component extends DCLogic {
  ...
  _cloudKeys(){ const local={deletedRoster:1,rosterEdited:1,rosterData:1,prospectData:1,contactsData:1,invoiceData:1,todoItems:1,briefItems:1,ideasData:1,events:1,threadMsgs:1,photos:1,authed:1,authRole:1,space:1,creatorId:1,portalTab:1}; return this._persistKeys().filter(k=> !local[k]); }
  _persist(){
  ...
  _mapCreator(r){ return { id:r.id, name:r.name, handle:r.handle, niche:r.niche, plat:r.platform, followers:r.followers, reach:r.reach, er:r.er, ca:r.ca, status:r.status, tone:r.tone, trend:r.trend, stats:r.stats||null, statsHistory:r.stats_history||[] }; }
  ...
  _loadAllData(){ try{ this._loadCreators(); this._loadProspects(); this._loadContacts(); this._loadInvoices(); this._loadTodos(); this._loadBriefs(); this._loadIdeas(); this._loadEvents(); this._loadMessages(); this._loadDocs(); }catch(e){} }
  ...
  renderVals(){
    ...
    const _mkIdea = (o,i)=>({ text:o.text, creatorLabel:o.creator||'Toutes', creator:o.creator||'Toutes', status:o.status, source:o.source==='creator'?'Proposée par le créateur':'Ajoutée par l’agence', fromCreator:o.source==='creator', dotStyle:dotS(ideaTone(o.status),false), statusStyle:"font:600 8px 'Inter',sans-serif;letter-spacing:.5px;padding:4px 9px;border-radius:20px;white-space:nowrap;color:"+this.toneHex(ideaTone(o.status),dark)+";background:"+this.toneHex(ideaTone(o.status),dark)+"18;", open:(()=>{const k=i;return ()=>this.setState({ideaOpen:k});})(), del:(e)=>{ if(e&&e.stopPropagation)e.stopPropagation(); if(!window.confirm('Supprimer cette idée ?'))return; if(this._ideasTable && o.id) this._dbDelete('ideas', o.id); this.setState(s=>({ ideasData:(s.ideasData||this.ideasRaw).filter(x=>x!==o), ideaOpen:null })); }, edit:(e)=>{ if(e&&e.stopPropagation)e.stopPropagation(); const nv=window.prompt("Modifier l'idée :", o.text); if(nv==null)return; const v=nv.trim(); if(!v)return; if(this._ideasTable && o.id) this._dbUpdate('ideas', o.id, {text:v}); this.setState(s=>({ ideasData:(s.ideasData||this.ideasRaw).map(x=> x===o ? Object.assign({},x,{text:v}) : x) })); } });
    ...
    const invoices = (this.state.invoiceData||this.invoiceRaw).filter(v=> _match(v.ref,v.party,v.amount)).map(v=>{ const st=this.invStatus(v.status); const url=_invDoc(v,st.label); const filename='facture-'+v.ref+'.html'; return { ref:v.ref, party:v.party, amount:v.amount, date:v.date, statusLabel:st.label, dotStyle:dotS(st.tone,false), chipStyle:this.chip(), url, filename, _hit:_match(v.ref,v.party,v.amount), open:(()=>{const u=url,fn=filename,rf=v.ref;return ()=>this.setState({previewDoc:{name:'Facture '+rf,url:u,filename:fn,isImage:false}});})(), share:(e)=>{ if(e&&e.stopPropagation)e.stopPropagation(); const txt='Facture '+v.ref+' — '+v.party+' — '+v.amount+' (échéance '+v.date+')'; if(navigator.share){navigator.share({title:'Facture '+v.ref, text:txt}).catch(()=>{});}else if(navigator.clipboard){navigator.clipboard.writeText(txt); this.setState({copied:'inv'+v.ref}); setTimeout(()=>this.setState(s=>s.copied==='inv'+v.ref?{copied:null}:{}),1500);}else{alert(txt);} }, shareLabel:(this.state.copied==='inv'+v.ref?'COPIÉ ✓':'PARTAGER'), del:(e)=>{ if(e&&e.stopPropagation)e.stopPropagation(); if(!window.confirm('Supprimer la facture '+v.ref+' ?'))return; if(this._invoicesTable && v.id) this._dbDelete('invoices', v.id); this.setState(s=>({ invoiceData:(s.invoiceData||this.invoiceRaw).filter(x=>x!==v) })); } }; });
    ...
    if(rdi!=null && this.rosterRaw[rdi]){ const c=this.rosterRaw[rdi]; const fn=c.name.split(' ')[0];
      ...
      const _mkRf=(label,key)=>({ label, value:rinfoBase[key]||'', onInput:mkRdInfo(key), copy:_rinfoCopy(label,rinfoBase[key]), canCopy:!!(rinfoBase[key]&&String(rinfoBase[key]).trim()), copyLabel:(this.state.rinfoCopied===label?'COPIÉ ✓':'COPIER') });
    ...
    const agencyDocs = (docBase[_docName]||[]).map((d,idx)=>mkDoc(d,_docName,idx));
    ...
    const addInvoice = () => {
      ...
      const ci=this.state.niFacCreator; const crName=(ci!=null && this.rosterRaw[ci]) ? this.rosterRaw[ci].name.split(' ')[0] : '';
      ...
      const row={ref, party, amount, date:due, status};
    ...
    return {
      ...
      addAccess:()=>{ const email=(this.state.acEmail||'').trim().toLowerCase(); const pwd=(this.state.acPwd||'').trim(); if(!email||!pwd){ toast('Email et mot de passe requis'); return; } const role=this.state.acRole||'creator'; const creator= role==='creator' ? (this.state.acCreator||(this.rosterRaw[0]||{}).name||null) : null; if((this.state.accessAccounts||[]).some(a=>String(a.email||'').toLowerCase()===email)){ toast('Cet email a déjà un accès'); return; }
        ...
        this.setState(s=>{ const upd={ accessAccounts:[...(s.accessAccounts||[]), {email,pwd,role,creator}], acEmail:'', acPwd:'' }; if(role==='creator' && creator){ const ri=Object.assign({},s.rosterInfo); ri[creator]=Object.assign({}, this.rosterInfoRaw[creator]||{}, ri[creator]||{}, {email}); upd.rosterInfo=ri; } return upd; });
      ...
      addIdea:()=>{ const t=(this.state.niText||'').trim(); if(!t)return; const item={text:t, creator:this.state.niCreator||null, status:this.state.niStatus||'\u00c0 explorer', source:'agency'}; const cur=(this.state.ideasData||this.ideasRaw).slice(); cur.unshift(item); this.setState({ideasData:cur, showIdeaForm:false, niText:''}); if(this._ideasTable){ this._dbInsert('ideas',{text:item.text,creator:item.creator,status:item.status,source:item.source,sort_order:0}).then(r=>{ if(r&&r.id){ item.id=r.id; this.setState({}); } }); } },
      addIdeaMe:()=>{ const t=(this.state.niText||'').trim(); if(!t)return; const ci=this.state.creatorId; const crr=(ci!=null?this.rosterRaw[ci]:null)||this.rosterRaw[0]; const nm=crr?crr.name:null; const item={text:t, creator:nm, status:'\u00c0 faire', source:'creator'}; const cur=(this.state.ideasData||this.ideasRaw).slice(); cur.unshift(item); this.setState({ideasData:cur, niText:''}); toast('Id\u00e9e envoy\u00e9e \u00e0 l\'agence \u2713'); if(this._ideasTable){ this._dbInsert('ideas',{text:item.text,creator:item.creator,status:item.status,source:item.source,sort_order:0}).then(r=>{ if(r&&r.id){ item.id=r.id; this.setState({}); } }); } },
      ...
      onAddDocMe:(e)=>{ const f=e.target.files&&e.target.files[0]; if(!f)return; const ci2=this.state.creatorId!=null?this.state.creatorId:0; const nm=(this.rosterRaw[ci2]||{}).name; if(!nm){ e.target.value=''; return; } this._addDocFor(nm, f, 'autre'); e.target.value=''; },
      ...
      doLogin: async () => {
          ...
          try {
            ...
            if(!error && data && data.user){
              ...
              if(agencyEmails.indexOf(uemail)>=0){ role='agency'; }
                ...
                this.rosterRaw.forEach((c,i)=>{ const seed=((this.rosterInfoRaw[c.name]||{}).email||''); const ov=((this.state.rosterInfo&&this.state.rosterInfo[c.name])||{}).email; const em=String(ov||seed||'').toLowerCase().trim(); if(em && em===uemail){ role='creator'; creatorId=i; } });
        ...
        if(acc){
          if(acc.role==='creator'){ const idx=this.rosterRaw.findIndex(c=>c.name===acc.creator); this.setState({ authed:true, authRole:'creator', space:'creator', creatorId:(idx>=0?idx:0), portalTab:'accueil', loginEmail:'', loginPwd:'', loginError:'', mobileNav:false }); }
      ...
      addEvent: () => { const ne=this.state.ne; if(!ne.title) return; const item={day:Number(ne.day)||26, time:ne.time||'—', title:ne.title, type:ne.type, who:null}; const cur=this.state.events||this.eventsRaw; this.setState({ events:[...cur, item], showEventForm:false, ne:{day:ne.day,time:'',title:'',type:'call'} }); if(this._eventsTable){ this._dbInsert('events',{day:item.day,time:item.time,title:item.title,type:item.type,who:item.who,sort_order:cur.length}).then(r=>{ if(r&&r.id){ item.id=r.id; this.setState({}); } }); } },
