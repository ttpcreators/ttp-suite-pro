...
class Component extends DCLogic {
  ...
  renderVals(){
    ...
    const myInvoices = _invMe.filter(v=> (v.creator&&v.creator===cr.name) || (!v.creator && (String(v.party||'').split(' × ')[1]||'').trim()===(cr.name||'').split(' ')[0] && cr.name) ).map(v=>{ const st=this.invStatus(v.status); return { ref:v.ref, party:v.party, amount:v.amount, date:v.date, statusLabel:st.label, dotStyle:dotS(st.tone,false), chipStyle:this.chip() }; });
