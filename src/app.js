...
class Component extends DCLogic {
  ...
  _loadAllData(){ try{ this._loadCreators(); this._loadProspects(); this._loadContacts(); this._loadInvoices(); this._loadTodos(); this._loadBriefs(); this._loadIdeas(); this._loadEvents(); this._loadMessages(); }catch(e){} }
  ...
  renderVals(){
    ...
    const mkTodo = (t, i) => {
      ...
      return {
        ...
        toggle: () => { const nv=!(this.state.doneSet && this.state.doneSet[i]); if(this._todosTable && t.id) this._dbUpdate('todos', t.id, {done:nv}); this.setState(s => ({ doneSet: Object.assign({}, s.doneSet, { [i]: nv }) })); },
    ...
    if(rdi!=null){ const c=this.rosterRaw[rdi]; const fn=c.name.split(' ')[0];
      ...
      const rinfoFields=[ _mkRf('Ville / Pays','ville'), _mkRf('Téléphone','phone'), _mkRf('Email','email'), _mkRf('Adresse postale','address'), _mkRf('SIREN / SIRET','siren'), _mkRf('Date de naissance','birth'), _mkRf('Commission TTP (%)','commission') ];
