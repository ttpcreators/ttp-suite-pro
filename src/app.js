...
class Component extends DCLogic {
  ...
  _cloudKeys(){ const local={deletedRoster:1,rosterEdited:1,prospectData:1,photos:1,authed:1,authRole:1,space:1,creatorId:1,portalTab:1}; return this._persistKeys().filter(k=> !local[k]); }
  _persist(){
  ...
  renderVals(){
    ...
    const addInvoice = () => {
      ...
      this.setState(s=>{ const base=(s.invoiceData||this.invoiceRaw); const ref='2026-'+String(180+base.length).padStart(3,'0'); return { invoiceData:[{ref, party, amount, date:due, status}].concat(base), showInvoiceForm:false, niFacBrand:'', niFacAmount:'', niFacDue:'', niFacCreator:null, niFacStatus:'brouillon' }; });
      toast('Facture créée ✓');
    };
    ...
    const addContact = () => {
      ...
      this.setState(s=>({ contactsData:[{brand:company, person:(person||'—'), role:(this.state.ncoRole||'').trim(), tag:type, email:(this.state.ncoEmail||'').trim(), phone:(this.state.ncoPhone||'').trim(), last:'jamais', deals:'—', tone:_typeTones[type]||'indigo'}].concat(this._contactsFrom(s)), showContactForm:false, ncoFirst:'', ncoLast:'', ncoCompany:'', ncoRole:'', ncoEmail:'', ncoPhone:'', ncoType:'Marque' }));
